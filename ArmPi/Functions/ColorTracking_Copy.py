...
def run(img):
    ...
    #If object is detected, keep detecting where it is until it is picked up
    ...
    if not start_pick_up:
        for i in color_range:
            if i in __target_color:
                ...
                areaMaxContour, area_max = getAreaMaxContour(contours)  # Get max area contour
        if area_max > 2500:  # If area over a threshold, then found the max area
            ...
            distance = math.sqrt(pow(world_x - last_x, 2) + pow(world_y - last_y, 2)) #Compare distance to last coordinates to decided whether or not to move
